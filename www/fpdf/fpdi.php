...
class FPDI extends FPDF_TPL
{
    ...
    public function importPage($pageNo, $boxName = 'CropBox', $groupXObject = true)
    {
        ...
        #$tpl['buffer'] = str_replace('{:pric)15(e-all:}', xsSecure::GET('price', 'int').'', $parser->getContent());
    ...
    public function replaceContent($search, $replace)
    {
        $tpl =& $this->_tpls[$this->tpl];
        #print_r($tpl);
        $tpl['buffer'] = str_ireplace($search, $replace, $tpl['buffer']);
        #echo $tpl['buffer'];


        #echo sprintf('Td (%s) Tj ET', $replace);



        #echo current(unpack('H*', $replace));

        #echo sprintf('%b', $replace);
