...
class TCPDF {
	 ...
	 * @public
	 */
	public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false, $saveProportions=false) {
		...
		if($saveProportions && $w > 0 && $h > 0)
		{
			$fullW = ($x2 - $x1) / $k;
			$fullH = ($y2 - $y1) / $k;

			$newW = ($x2 - $x1) / $k * ($h / (($y2 - $y1) / $k));
			$newH = ($y2 - $y1) / $k * ($w / (($x2 - $x1) / $k));
